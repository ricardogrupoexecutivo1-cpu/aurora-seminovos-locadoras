...
function buildFallbackSlug(company: CompanyItem) {
...
function getCanonicalSlug(company: CompanyItem) {
  return company.slug?.trim() || buildFallbackSlug(company);
}
...
export default function EmpresaSlugPage() {
  ...
  useEffect(() => {
    ...
    async function loadCompany() {
      try {
        ...
        const found =
          companies.find((item) => item.slug === slug) ||
          companies.find((item) => buildFallbackSlug(item) === slug) ||
          null;
        ...
        const canonicalSlug = getCanonicalSlug(found);

        if (canonicalSlug && canonicalSlug !== slug) {
          if (!active) return;
          setRedirecting(true);
          router.replace(`/seminovos-locadoras/empresa/${canonicalSlug}`);
          return;
        }
  ...
  const canonicalSlug = useMemo(
    () => (company ? getCanonicalSlug(company) : ""),
    [company]
  );
          ...
          {loading || redirecting ? (
            <div className="p-6 sm:p-8">
              <div className="rounded-[28px] border border-cyan-100 bg-cyan-50 p-6 text-sm text-cyan-700">
                {redirecting
                  ? "Redirecionando para o slug atual da empresa..."
